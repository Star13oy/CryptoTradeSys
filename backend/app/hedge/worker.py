...
from .schemas import HedgeRebalanceWorkerSnapshot
from .service import HedgeManagerService


class HedgeRebalanceWorker:
    def __init__(
        self,
        ...
        *,
        ...
        self._snapshot = HedgeRebalanceWorkerSnapshot(
            enabled=enabled,
    ...
    async def start(self) -> None:
        if not self._enabled or self._orchestrator is None or self._task is not None:
            return
        self._task = asyncio.create_task(self._run_loop(), name="hedge-rebalance-worker")
    ...
    async def run_once(self) -> HedgeRebalanceWorkerSnapshot:
        if self._orchestrator is None:
            raise ValueError("hedge rebalance worker is not configured with an execution orchestrator")
        ...
        async with self._lock:
            ...
            try:
                summary = await asyncio.to_thread(
                    self._service.execute_auto_rebalance_actions,
            ...
            self._snapshot.total_executed_rebalances += summary.executed_count
            return self.snapshot()
